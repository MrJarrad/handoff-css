...
A document that names `schemaVersion: 8` is checked against
`schema/design-system-handoff.v8.schema.json` before a line of CSS exists, and
...
says `skipped: true` rather than `ok` with no checks run. Dropped in 0.4.0.
